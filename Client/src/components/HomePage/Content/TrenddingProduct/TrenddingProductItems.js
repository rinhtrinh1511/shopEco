...
class TrenddingProductItems extends Component {
  ...
  addItemToFavorite = (id) => {
    startLoading();
    if (!token) {
      return toast.error("Đăng nhập trước khi thêm vào danh sách yêu thích");
    }
  ...
  render() {
    ...
    if (product.rating && product.rating.length > 0) {
      let totalRating = 0;
      let count = 0; // Thêm biến count để đếm số lượng đánh giá
      
      product.rating.forEach((item) => {
        count++;
        totalRating += item.point;
      });
      
      const sumRating = Math.round(totalRating / count);
      // Sử dụng const thay vì sumRating để khai báo biến với phạm vi block (block scope)
      
      console.log(sumRating); // Để làm gì đó với sumRating
    }
    return (
      <div>
